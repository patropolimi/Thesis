...
""" Launch Script For Poisson 2D [To Be Launched Once For Each Test] [To Be Tuned By Hand For Every Launch] """


def F(X):
	return ((-18*jnp.pi**2)*jnp.sin(3*jnp.pi*X[0,:])*jnp.sin(3*jnp.pi*X[1,:]))[None,:]
def S(X):
	return (jnp.sin(3*jnp.pi*X[0,:])*jnp.sin(3*jnp.pi*X[1,:]))[None,:]
SolutionString='1*sin(3*pi*x)*sin(3*pi*y)'
