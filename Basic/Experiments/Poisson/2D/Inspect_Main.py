...
def main():
	while (True):
		...
		if (Continue):
			...
			XG,YG=np.meshgrid(Points_1D,Points_1D)
			Axes=plt.axes(projection='3d')
			Axes.plot_surface(XG,YG,Model['Network_Eval'].reshape((N_1D,N_1D)))
			Axes.plot_surface(XG,YG,Model['Solution_Eval'].reshape((N_1D,N_1D)))
			plt.show()
