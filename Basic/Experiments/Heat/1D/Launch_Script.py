...
""" Launch Script For Heat 1D [To Be Launched Once For Each Test] [To Be Tuned By Hand For Every Launch] """
...
SolutionString='1*exp(-t)*sin(3*pi*x)'

def G(X):
	return jnp.where(jnp.abs(X[0,:])<EpsMachine,jnp.sin(3*jnp.pi*X[1,:]),jnp.sum(jnp.zeros_like(X),axis=0))[None,:]

Test=1
NAttempts=3
Number_Residuals=[250]
...
Number_Boundary_Points=[[200,0],[50,50]]
		...
		for HL in Hidden_Layers:
			for NPL in Neurons_Per_Layer:
					for i in range(NAttempts):
						...
						Data={'Source': F,'Exact_Dirichlet': G,'Exact_Neumann': G,'Nu': 1.0/(3*jnp.pi)**2}
						Solver=Resolutor_Basic[Heat_Scalar_Basic](Architecture,Domain,Data)
