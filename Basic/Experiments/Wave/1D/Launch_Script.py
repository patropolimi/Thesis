...
""" Launch Script For Wave 1D [To Be Launched Once For Each Test] [To Be Tuned By Hand For Every Launch] """


def F(X):
	return jnp.sum(jnp.zeros_like(X),axis=0)[None,:]
def S(X):
	return (jnp.sin(jnp.pi*X[1,:])*jnp.cos(5*jnp.pi*X[0,:]))[None,:]
SolutionString='1*sin(pi*x)*cos(5*pi*t)'

def G(X):
	return jnp.where(jnp.abs(X[0,:])<EpsMachine,jnp.sin(jnp.pi*X[1,:]),jnp.sum(jnp.zeros_like(X),axis=0))[None,:]

Test=1
NAttempts=3
Number_Residuals=[900]
...
Number_Boundary_Points=[[150,0],[50,50]]
Boundary_Labels=['Dirichlet','None','Dirichlet','Dirichlet']
Number_Initial_Velocity_Points=100
Hidden_Layers=[1]
		...
		for HL in Hidden_Layers:
			for NPL in Neurons_Per_Layer:
					for i in range(NAttempts):
						...
						Domain={'Dimension': 2,'Limits': Limits,'Number_Residuals': NR,'Number_Boundary_Points': Number_Boundary_Points,'Boundary_Labels': Boundary_Labels,'Number_Initial_Velocity_Points': Number_Initial_Velocity_Points}
						Data={'Source': F,'Exact_Dirichlet': G,'Exact_Neumann': F,'C': 5.0}
						Solver=Resolutor_Basic[Wave_Scalar_Basic](Architecture,Domain,Data)
