...
for SigmaName in Activations:
	for Frequency in Frequencies:
				...
				for i,NPL in enumerate(Neurons_Per_Layer):
					for j,HL in enumerate(Hidden_Layers):
						...
						for NA in range(NAttempts):
							LookName='./Models_'+SigmaName+'/'+'Test_'+str(Test)+'/'+'Model_'+Frequency+'_'+str(HL)+'HL_'+str(NPL)+'NPL_'+str(NR)+'NR_'+str(NA+1)
		...
		SaveName='./Models_'+SigmaName+'/'+'Test_'+str(Test)+'/'+'Models_'+Frequency+'_Tables'
