...
""" Organize Main To Write Summary Error Tables For Models Created For Basic PINN Single-Scale Sensitivity Analysis """
...
def main():
	...
	for F in Freqs:
				...
				for i,NPL in enumerate(NeursPerLay):
					for j,HL in enumerate(HiddLays):
						...
						for NA in range(NAtts):
							LookName='Test_'+str(T)+'/'+Act+'_Model_'+F+'_'+str(HL)+'HL_'+str(NPL)+'NPL_'+str(NR)+'NR_'+str(NA+1)
		...
		SaveName='Test_'+str(T)+'/'+Act+'_Models_'+F+'_Tables'
