...
""" Test 5 [Part 2] Launch Script -> Basic PINN Generic Sensitivity Analysis"""
...
def S(X):
	...
	for i in range(N-1):
		Y=H(Y)
	return Y[None,:]
String=str(2**(N-1))+'Teeth_Saw_Travelling_[0,1]x[0,1]'
def F(X):
	return jnp.zeros_like(X)
def G(X):
	return jnp.where(X[0,:]<EpsMachine,S(X),jnp.zeros_like(X))
NAttempts=3
Number_Residuals=[250,500]
ADAM_Steps=50000
LBFGS_MaxSteps=150000
