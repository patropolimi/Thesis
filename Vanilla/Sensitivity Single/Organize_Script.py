...
for SigmaName,Sigma in Activations.items():
	for Frequency in Frequencies:
				...
				for i in range(Neurons_Per_Layer_Options):
					for j in range(Hidden_Layers_Options):
						...
						for NA in range(NAttempts):
							LookName='./Models_'+SigmaName+'/'+'Test_'+str(Test)+'/'+Frequency+'_'+str(Hidden_Layers[j])+'HL_'+str(Neurons_Per_Layer[i])+'NPL_'+str(NR)+'NR_'+str(NA+1)
		...
		SaveName='./Models_'+SigmaName+'/'+'Test_'+str(Test)+'/'+Frequency+'_Tables'
